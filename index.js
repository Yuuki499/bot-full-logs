...
const client = new Discord.Client({ partials: ["GUILDS", "GUILD_MESSAGE_REACTIONS", "GUILD_MESSAGES", "GUILD_INVITES", "GUILD_VOICE_STATES", "GUILD_MEMBERS", "GUILD_PRESENCES"], intents: 32767 });
...
client.on("messageUpdate", (oldMessage, newMessage) => {
    ...
    let log = new Discord.MessageEmbed()
        .setTitle("```[ LOGS ] ```")
        .setDescription(
            " \n\n <:hashtagperigo:894551117466329128>  Mensagem Antes: " +
                `\`\`\`${oldMessage}\`\`\`` +
                "\n\n <:hashtagperigo:894551117466329128>  Mensagem Depois: " +
                `\`\`\`${newMessage}\`\`\`
...
client.on("inviteCreate", (invite) => {
    ...
    let log = new Discord.MessageEmbed()
        .setTitle("```[ LOGS ] ```")
        .setDescription(
            " \n\n <:hashtagperigo:894551117466329128>  Invite criado: " +
                `${invite}` +
...
client.on("messageReactionAdd", (messageReaction, user) => {
    ...
    let log = new Discord.MessageEmbed()
        .setTitle("```[ LOGS ] ```")
        .setDescription(
            " \n\n <:hashtagperigo:894551117466329128>  Reação adicionada: " +
...
client.on("messageReactionRemove", (messageReaction, user) => {
    ...
    let log = new Discord.MessageEmbed()
        .setTitle("```[ LOGS ] ```")
        .setDescription(
            " \n\n <:hashtagperigo:894551117466329128>  Reação removida: " +
                `${messageReaction.emoji} \n<:casa:894551321401765928> User: ${user}  \n <:casa:894551321401765928> Ir para mensagem  [clique aqui](https://discord.com/channels/${messageReaction.message.guild.id}/${messageReaction.message.channel.id}/${messageReaction.message.id})`,
        )
...
client.on("userUpdate", (oldUser, newUser, guild) => {
    ...
    let log = new Discord.MessageEmbed()
        ...
        .setDescription(
            `\n\n ${oldUser} ` +
                " Membro antes: " +
                `\`${oldUser.username}\`// Tag: \`${oldUser.discriminator}\`
                                                            \n\n ${newUser} ` +
                " Membro depois: " +
                `\`${newUser.username}\`// Tag: \`${newUser.discriminator}\``,
        )
...
client.on("voiceStateUpdate", (oldState, newState) => {
    ...
    let log = new Discord.MessageEmbed()
        ...
        .setDescription(
            `\n\n ${oldState.member} ` +
                " Membro antes: " +
                `\` Mutado geral: ${oldState.selfDeaf}\`// \` Mutado: ${oldState.selfMute}\`// \` Vídeo: ${oldState.selfVideo}\`//\`Mutado geral pela guild: ${oldState.serverDeaf}\`//\`Mutado pela guild:  ${oldState.serverMute}\` // \`call:\` ${oldState.channel}
                                                                \n\n ${newState.member} ` +
                " Membro depois: " +
                `\` Mutado geral: ${newState.selfDeaf}\`//\`Mutado: ${newState.selfMute}\`//\` Vídeo: ${newState.selfVideo}\`//\`Mutado geral pela guild: ${newState.serverDeaf}\`//\`Mutado pela guild: ${newState.serverMute}\` // \`call:\`${newState.channel}`,
        )
